...
|10:00-12:00|复习es的查询操作|match<br>bool<br>should<br>term<br>等|
|14:00-17:00|复习es的分布式实现|集群<br>节点<br>shard<br>replic<br>查询过程<br>分片和副本的优点<br>document更新过程|
|18:00-19:00|了解es的8种搜索方式||
|2020|05|27|
|10:00-12:30|复习redis的启动参数和数据的使用命令||
|14:30-19:00|leetcode1323题|还没有想到更合理的方法解决,还在想|
|2020|05|28|
|11:00-18:00|leetcode1323题|用数字转换字符串的方法尝试了很转换方式,都没有成功,在五点的时候选择看了题解,发现都是默认只有四位数的解题方法,仔细一看三个示例下方给的提示给出了数字范围,只要过一遍四个数字把第一个出现的6改成9即可|
|19:00-21:00|整理ssh、git命令||
|2020|06|05|
|了解shell|解决报错问题`unexpected operator`|默认的dash改回bash符号改成bash形式|
|解决mount问题|挂载后只有root有WX权限||
|2020|06|09|
|换种思路做21题移除元素|执行用时0ms内存消耗2.1MB|之前没有想到还有这样的思路,按照题目的说法想不到在发现重复元素直接返回当前index|
|明日计划|继续尝试第28题||
|看28思路尝试做|暂时没做出来||
